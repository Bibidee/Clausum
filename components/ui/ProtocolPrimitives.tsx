...
export function HashDisplay({ label = "SHA-256", value, compact = false }: { label?: string; value: string; compact?: boolean }) { const shown = compact && value && value.length > 20 ? `${value.slice(0, 10)}…${value.slice(-8)}` : value || "not available"; return <code className="hash-display" title={value || undefined}>{label} · {shown}</code>; }
...
export function ExplorerLink({ value, kind = "tx" }: { value: string; kind?: "tx" | "address" }) { if (!value) return null; const href = `https://explorer-studio.genlayer.com/${kind}/${value}`; return <a className="external-link" href={href} target="_blank" rel="noreferrer">Explorer <ExternalLink size={13} /></a>; }
